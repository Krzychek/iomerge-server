...
public class PluginLoader {
	...
	public void loadPluginsToContext(AnnotationConfigApplicationContext ctx) {
		if (!ConstantPaths.PLUGINS_DIR.isDirectory()) {
			Logger.info("Plugin dir doesn't exists: ", ConstantPaths.PLUGINS_DIR);
			return;
		}
	...
	private Stream<File> getPluginsJars() {
		File[] plugins = ConstantPaths.PLUGINS_DIR.listFiles(file -> file.isFile() && file.getName().endsWith(".jar"));
